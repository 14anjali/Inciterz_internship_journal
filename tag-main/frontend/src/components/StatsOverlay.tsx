...
export const StatsOverlay = ({ className }: { className?: string }) => {
    ...
    const handleStatsUpdate = (newStats: StatsData) => {
      if (newStats && typeof newStats.users === "number") {
        setStats((prev) => ({
          ...
          ...newStats,
          plants:
            newStats.plants !== undefined ? newStats.plants : prev.plants,
          forumPosts:
            newStats.forumPosts !== undefined
              ? newStats.forumPosts
              : prev.forumPosts,
        }));
      }
  ...
  const cardStyle =
    "dark:bg-[#2C354B] bg-white rounded-lg shadow-md p-4 flex flex-col items-center justify-center dark:text-white text-foreground";

  const StatCard = ({ icon: Icon, value, label, className }: any) => (
    <div
      className={`${cardStyle} ${className || ""} min-w-[110px] min-h-[120px]`}
    >
      <Icon className="h-8 w-8 text-primary mb-2" />
      <span className="text-2xl font-bold leading-none mb-1">{value}</span>
  ...
  return (
    <div
      className={`
        lg:dark:bg-[#151F37]/60 lg:bg-background/60 lg:backdrop-blur-xl 
        lg:rounded-tr-[40px] lg:border-t-4 lg:border-r-4 lg:dark:border-white/30 lg:border-black/10 lg:shadow-2xl lg:p-6
        w-full lg:w-auto bg-black/60 backdrop-blur-md p-2
        ${className || ""}
      `}
      style={{ backdropFilter: "blur(20px) saturate(180%)" }}
    >
      <h3 className="text-lg font-semibold mb-4 dark:text-white text-foreground text-center hidden lg:block">
      ...
      <div className="grid grid-cols-3 gap-2 lg:hidden w-full">
        <div className="flex flex-col items-center justify-center p-1">
          ...
          <span className="text-[9px] text-white/80 font-medium text-center leading-none mt-0.5">
            Users
          </span>
        </div>
