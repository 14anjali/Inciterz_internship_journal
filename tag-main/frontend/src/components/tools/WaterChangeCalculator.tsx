...
export const WaterChangeCalculator = ({ children }: WaterChangeCalculatorProps) => {
  ...
  }, [foodWeightGrams, proteinPercentage, tankVolume,currentNitrate]);
  ...
  const isDisabled =
  !tankVolume ||
  parseFloat(tankVolume) <= 0 ||
  (mode === "simple" && (!simpleGoal || parseFloat(simpleGoal) <= 0)) ||
  (mode === "redfield" && (!phosphate || parseFloat(phosphate) <= 0));
  return (
    <Dialog>
      <DialogTrigger asChild>{children}</DialogTrigger>
      <DialogContent className="w-[95vw] sm:w-full sm:max-w-5xl max-h-[95vh] overflow-y-auto">
        <DialogHeader>
