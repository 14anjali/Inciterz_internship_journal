    ...
    const handleSubmit = async () => {
        try {
            ...
            if (formData.category === "fish") {
                ...
                await speciesApi.addSpecies(fishData as any);
            } else {
                const plantData = {
                    ...
                    water_type: formData.waterType || "freshwater",
                    family: formData.family,
                    origin: formData.origin,
                ...
                await speciesApi.addSpecies(plantData as any);
            }
            ...
            <DialogContent className="w-[95vw] sm:w-full max-w-4xl h-[90vh] p-0 bg-card border-border flex flex-col">
                <DialogHeader className="p-6 pb-2">
                    ...
                    <div className="mt-4 p-4 bg-secondary/20 rounded-lg border border-border">
                        <Label className="text-xs font-bold uppercase mb-2 block">Primary Classification</Label>
                        <Select value={formData.category} onValueChange={(v) => setFormData((prev) => {
                            if (v === "fish") return { ...prev, category: v };
                            const parsed = parseScientificName(prev.scientificName);
                            return { ...prev, category: v, ...parsed };
                        })}>
                            <SelectTrigger className="w-full bg-background"><SelectValue /></SelectTrigger>
                            <SelectContent>
                                <SelectItem value="fish">Fish (Fauna)</SelectItem>
                                <SelectItem value="aquaticplants">Plants (Flora)</SelectItem>
                                <SelectItem value="macroalgae">Algae</SelectItem>
                            </SelectContent>
                        </Select>
                    </div>
                </DialogHeader>
                ...
                <Tabs defaultValue="basic" className="flex-1 flex flex-col overflow-hidden">
                        ...
                        <div className="p-6">
                                    ...
                                    {formData.category === "fish" ? (
                                        <>
                                            ...
                                            <div className="space-y-2 sm:col-span-2">
                                                <Label>Varieties (e.g. Orandas, Ranchus)</Label>
                            ...
                            {formData.category === "fish" && (
                                <TabsContent value="care" className="mt-0 space-y-6">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Select value={formData.bioload} onValueChange={(v) => handleInputChange("bioload", v)}>
                                                <SelectTrigger><SelectValue placeholder="Select bioload" /></SelectTrigger>
                                                <SelectContent>
                                    ...
                                    <div className="space-y-2"><Label>Plant Safety</Label><Input value={formData.plantSafety} onChange={(e) => handleInputChange("plantSafety", e.target.value)} /></div>
                            ...
                            {/* HEALTH TAB (FISH ONLY) */}
                            {formData.category === "fish" && (
                                <TabsContent value="health" className="mt-0 space-y-4">
                                    <div className="flex justify-between items-center">
                                        <h3 className="font-semibold">Health Watch</h3>
                                        <Button onClick={addDisease} size="sm" variant="outline"><Plus className="w-4 h-4 mr-2" /> Add Disease</Button>
                                    </div>
                                    {formData.commonDiseases.map((d, i) => (
                                        <div key={i} className="p-4 border rounded-lg relative bg-muted/20 space-y-3">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pr-8">
                                                <div className="space-y-2"><Label>Disease Name</Label><Input value={d.condition} onChange={(e) => updateDisease(i, "condition", e.target.value)} /></div>
                                                <div className="space-y-2"><Label>Cause</Label><Input value={d.potential_cause} onChange={(e) => updateDisease(i, "potential_cause", e.target.value)} /></div>
                                                <div className="space-y-2 md:col-span-2"><Label>Symptoms</Label><Textarea value={d.symptoms} onChange={(e) => updateDisease(i, "symptoms", e.target.value)} /></div>
                                            </div>
                                        </div>
                                    ))}
                                </TabsContent>
                                ...
                                <TabsContent value="breeding" className="mt-0 space-y-4">
                                    ...
                                    {formData.fryDevelopment.map((s, i) => (
                                        <div key={i} className="grid grid-cols-1 md:grid-cols-3 gap-2 border p-4 rounded-lg relative bg-muted/10">
                                            ...
                                            <div className="space-y-1"><Label className="text-xs">Timeframe</Label><Input value={s.timeframe} onChange={(e) => updateFryStage(i, "timeframe", e.target.value)} /></div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex gap-2">
                                        <div className="relative flex-1">
                                            ...
                                            <Input className="pl-10" placeholder="Image URL" value={formData.imageUrl} onChange={(e) => handleInputChange("imageUrl", e.target.value)} />
                                        </div>
                                        <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFileSelect} />
                                        <Button type="button" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={isUploading}>
                                            {isUploading ? <Loader2 className="animate-spin" /> : <Upload className="mr-2 h-4 w-4" />} Browse
                                        </Button>
                                    </div>
                                    {formData.imageUrl && (
                    ...
                    <div className="flex gap-3 p-6 border-t border-border mt-auto">
                        ...
                        <Button onClick={handleSubmit} className="flex-1" disabled={isLoading}>
                            {isLoading ? <Loader2 className="animate-spin mr-2" /> : null} Save {formData.category === 'fish' ? 'Fish' : 'Flora'}
                        </Button>
                    </div>
                </Tabs>
            </DialogContent>
