...
import ViewVideoGuide from "./pages/ViewVideoGuide";
...
import CreateTextGuide from "./pages/CreateTextGuide";
import CreateForumDiscussion from "./pages/CreateForumDiscussion";
import SearchResults from "./pages/SearchResults";
import ResetPassword from "./pages/ResetPassword";
import { AdsVisibilityProvider } from "./context/AdsVisibilityContext";
...
const App = () => {
  ...
  useEffect(() => {
    const verifySession = async () => {
      // Only verify if we have both userid AND a token in storage
      // This prevents sending requests without tokens (which cause 401s/logs)
      ...
      <TooltipProvider>
        ...
        <BrowserRouter>
          ...
          <Routes>
            ...
            <Route element={<Layout><CreateTextGuide /></Layout>} path="/text-guides/create" />
            ...
            <Route element={<Layout><CreateForumDiscussion /></Layout>} path="/community-forum/create" />
			...
			<Route element={<Layout><WaterGuide /></Layout>} path="/water-guide" />
            <Route element={<Layout><SearchResults /></Layout>} path="/search" />
            ...
            <Route element={<Layout><ViewVideoGuide /></Layout>} path="/view/video/:id" />

            {/* PROTECTED ROUTES (Logged in Users Only) */}
            <Route element={<PrivateRoute />}>
              <Route element={<Layout><Profile /></Layout>} path="/profile" />
              
              {/* ADMIN ONLY ROUTES - URL reflects section: /admin/dashboard, /admin/manage-faq, etc. */}
              {(role === "admin" || role === "support") ? (
                ...
                <>
                  ...
                  <Route path="/admin/:section" element={<Navigate to="/" replace />} />
                </>
              )}
            </Route>
