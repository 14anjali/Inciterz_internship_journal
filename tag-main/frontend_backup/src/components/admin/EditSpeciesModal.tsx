...
const EditSpeciesModal = ({isOpen, onClose, species}: EditSpeciesModalProps) => {
	...
	const [formData, setFormData] = useState({
		...
		breedingDifficulty: "",
		breedingNotes: "",
		isFinNipper: "false",      // Added for compatibility logic
		hasDelicateFins: "false",  // Added for compatibility logic
		swimmingLevel: "middle",   // Added for territorial logic
		lifespanYears: "",
		socialNeeds: "",
	...
	useEffect(() => {
		if (species) {
			setFormData({
				...
				swimmingLevel: species.swimming_level?.toLowerCase() || "middle",
	...
	const handleSubmit = async () => {
		...
		try {
			...
			await speciesApi.updateSpecies(species.fish_id, speciesData);
			...
			toast.error(err instanceof Error ? err.message : "Failed to update species");
		} finally {
			setIsLoading(false);
		}
	};
