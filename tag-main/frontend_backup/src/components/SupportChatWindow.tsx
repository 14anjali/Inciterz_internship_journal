...
import { X, Send, Minus, Loader2, CheckCircle } from "lucide-react";
...
import { getSupportChatMessages, resolveSupportChat, SupportChatMessage } from "@/api/support";
...
const SupportChatWindow: React.FC<SupportChatWindowProps> = ({
    ...
    const [resolvedByUser, setResolvedByUser] = useState(false);
    ...
    useEffect(() => {
        ...
        const handleIncomingMessage = (msg: SupportChatMessage) => {
            ...
            try {
                ...
                if (response.success && response.messages) {
                    const formatted = response.messages.map((msg) => ({
                        id: msg.id,
                        text: msg.message,
                        sender: (msg.sender_id === userId ? "user" : "support") as "user" | "support",
                        timestamp: new Date(msg.created_at),
                    }));
                ...
                onSupportMessage(handleIncomingMessage);
        ...
        return () => {
            isMounted = false;
            offSupportMessage(handleIncomingMessage);
    ...
    const handleResolveChat = async () => {
        if (isResolved || isResolving || !chatId) return;
        try {
            setIsResolving(true);
            await resolveSupportChat(chatId);
            setIsResolved(true);
            setResolvedByUser(true);
            setMessages((prev) => [
                ...prev,
                {
                    id: "resolved-by-user",
                    text: "You have resolved this chat.",
                    sender: "support",
                    timestamp: new Date(),
                },
            ]);
            disconnectSupportSocket();
            // Close the widget after a short delay so the user sees the resolved state
    ...
    return (
        <Card className="w-[90vw] sm:w-[380px] shadow-2xl border-primary/20 animate-in slide-in-from-bottom-10 fade-in duration-300">
            ...
            <CardFooter className="p-3 bg-background border-t pt-3 flex flex-col gap-2">
                {isResolved ? (
                    <div className="w-full text-center text-sm text-muted-foreground">
                        {resolvedByUser ? "You have resolved this chat." : "This chat has been resolved by support."}
                    </div>
                ) : (
                    <>
                        <form
                        ...
                        >
                            ...
                            {isResolving ? "Resolving..." : "Resolve chat"}
                        </Button>
                    </>
                )}
            </CardFooter>
