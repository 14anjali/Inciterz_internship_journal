...
const CommunityForum = () => {
  ...
  const dummyPosts = [
    { id: "1", title: "Aquascaping Masterclass (Pt. 3): Substrate & Nutrition | TAG", Creator_Username: "nikhileshreddy", Total_Comments: 1, likes: { length: 10 } },
    { id: "2", title: "Beginner's Guide to Planted Tanks", Creator_Username: "aquaexpert", Total_Comments: 5, likes: { length: 25 } },
    { id: "3", title: "Advanced Filtration Systems Explained", Creator_Username: "fishkeeper", Total_Comments: 3, likes: { length: 15 } },
    { id: "4", title: "Water Chemistry Basics for Beginners", Creator_Username: "aquarist", Total_Comments: 8, likes: { length: 42 } },
    { id: "5", title: "Breeding Tropical Fish: Complete Guide", Creator_Username: "breederpro", Total_Comments: 12, likes: { length: 67 } },
    { id: "6", title: "Setting Up Your First Saltwater Tank", Creator_Username: "marineexpert", Total_Comments: 6, likes: { length: 33 } },
    { id: "7", title: "Plant Nutrition and CO2 Injection Guide", Creator_Username: "plantmaster", Total_Comments: 4, likes: { length: 28 } },
    { id: "8", title: "Common Fish Diseases and Treatments", Creator_Username: "vetexpert", Total_Comments: 15, likes: { length: 89 } },
  ] as any[];

  const displayPosts = forumPosts.length > 0 ? forumPosts : dummyPosts;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        {displayPosts.slice(0, 8).map((post, index) => (
          <Card
            ...
            onClick={() => {
              const postId = post.id || String(index + 1);
              navigate(`/view/forum/${postId}`);
            }}
          >
            ...
            <CardContent className="pt-0">
              <div className="flex items-center justify-end gap-4 text-xs text-muted-foreground">
                <div className="flex items-center gap-1">
                  ...
                  <span>{String(post.Total_Comments || '01').padStart(2, '0')}</span>
                </div>
                <div className="flex items-center gap-1">
                  <ThumbsUp className="h-3 w-3" />
                  <span>{post.likes?.length || '10'}</span>
                </div>
              </div>
            </CardContent>
          </Card>
