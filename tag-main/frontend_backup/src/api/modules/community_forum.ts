...
export const community_forum_api = {
    ...
    getAllApprovedCommunityForums: async (page: number, search?: string): Promise<getAllCommunityForum> => {
        const searchParam = search ? `&search=${encodeURIComponent(search)}` : "";
        const res = await httpClient.get<getAllCommunityForum>(`${COMMUNITY_URLS.GET_ALL_APPROVED}?page=${page}${searchParam}`);
        return res.data;
    },
