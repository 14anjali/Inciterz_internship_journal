...
export const get_text_guide = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    const limit = 12; // Increased from 2 to show more items per page
    const offset = (page - 1) * limit;
  ...
  try {
    ...
    if (status === "approved") {
      await createNotification({
        type: "text_guide",
        referenceId: text_guide.id,
        title: `New text guide: ${title}`,
        linkPath: `/view/text/${text_guide.id}`,
      });
    }
...
export const approve_or_reject_text = async (req, res) => {
  try {
    ...
    if (status === "approved") {
      const approved = await TextModel.findAll({ where: { id: ids }, attributes: ["id", "title"] });
      for (const g of approved) {
        await createNotification({
          type: "text_guide",
          referenceId: g.id,
          title: `New text guide: ${g.title}`,
          linkPath: `/view/text/${g.id}`,
        });
      }
    }
  ...
  try {
    ...
    if (status === "rejected") {
      ...
      await createNotification({
        type: "text_guide",
        referenceId: text_guide.id,
        title: `New text guide: ${text_guide.title}`,
        linkPath: `/view/text/${text_guide.id}`,
      });
    }
    ...
    for (const guide of guides) {
      if (action === "approve") {
        ...
        await createNotification({
          ...
          title: `New text guide: ${guide.title}`,
          linkPath: `/view/text/${guide.id}`,
        });
