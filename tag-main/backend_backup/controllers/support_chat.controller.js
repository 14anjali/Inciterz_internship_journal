...
// Helper: attach a requester_display_name field derived from initiator/support_members -> users
const buildChatWithRequesterName = (chatInstance) => {
  if (!chatInstance) return chatInstance;

  const plain = typeof chatInstance.toJSON === "function"
    ? chatInstance.toJSON()
    : chatInstance;

  const initiator = plain.initiator ?? plain.Initiator ?? null;
  const supportMembers = Array.isArray(plain.supportMembers ?? plain.SupportMembers)
    ? (plain.supportMembers ?? plain.SupportMembers)
    : [];
  const initiatedBy = plain.initiated_by;

  let requesterUser = initiator || null;

  if (!requesterUser && supportMembers.length > 0) {
    const fromMembers = supportMembers.find((m) => {
      const memberUserId = m?.user_id || m?.userId;
      const role = m?.user?.role;
      if (initiatedBy && memberUserId === initiatedBy) return true;
      return role === "user";
    });
    if (fromMembers && fromMembers.user) {
      requesterUser = fromMembers.user;
    }
  }

  const name = (requesterUser?.name || "").trim();
  const userid = (requesterUser?.userid || "").trim();

  const requester_display_name = name || userid || "Unknown User";
...
export const startChat = async (req, res) => {
  try {
    ...
    // Keep requester in support_members as well so requester name can be resolved from support_members -> users.
    ...
    // Reload the chat with initiator details so admin dashboard can show the user's full name
    const populatedChat = await SupportChat.findByPk(newChat.id, {
      include: [
        {
          model: User,
          as: "initiator",
          attributes: ["id", "name", "userid", "last_seen"],
        },
        {
          model: SupportMember,
          as: "supportMembers",
          attributes: ["id", "user_id", "support_chat_id", "is_locked"],
          include: [
            {
              model: User,
              as: "user",
              attributes: ["id", "name", "userid", "role", "last_seen"],
            },
          ],
        },
      ],
    ...
    const chatPayload = buildChatWithRequesterName(populatedChat ?? newChat);

    // Broadcast to all connected support/admin clients so the new chat appears in real-time
    try {
      const io = req.app.get("io");
      if (io) {
        io.of("/support").emit("support:new_chat", { chat: chatPayload });
      }
    } catch (socketError) {
      console.error("Error emitting support:new_chat:", socketError);
      // Do not fail the request just because the socket broadcast failed
    }

    res.status(201).json({ success: true, chat: chatPayload });
  }
  catch (error) {
...
export const acceptChat = async (req, res) => {
  try {
    ...
    let supportMember = await SupportMember.findOne({
      where: {
        user_id: user.id,
    ...
    if (!supportMember) {
      supportMember = await SupportMember.create({
        user_id: user.id,
        support_chat_id: chat.id,
        is_locked: false,
      });
...
export const getSupportChats = async (req, res) => {
  try {
    ...
    const chatsWithNames = chats.map(buildChatWithRequesterName);

    res.status(200).json({ success: true, chats: chatsWithNames });
  } catch (error) {
...
export const getSupportChatMessages = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      messages,
      isLocked: member ? member.is_locked : false,
    });
...
export const get_unaccepted_chats = async (req, res) => {
  try {
    ...
    const chatsWithNames = unacceptedChats.map(buildChatWithRequesterName);
