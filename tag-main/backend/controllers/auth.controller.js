...
import { getClientIp, getGeoFromRequest, getGeoLocation } from "../utils/location.util.js";
...
export const signup = async (req, res) => {
  try {
    ...
    let location = await getGeoLocation(ipAddress);
    
    // Fallback to Cloudflare Country if precise location failed
    if (!location && req.headers["cf-ipcountry"]) {
         location = {
    ...
    if (user.role === "user") {
      try {
        ...
        console.error("Guest conversion tracking error:", conversionError);
      }
    }
    ...
    // 📧 Send Welcome Email
    try {
      const guideLink = process.env.CLIENT_URL ? `${process.env.CLIENT_URL}/guides/nitrogen-cycle` : "http://localhost:8080/guides/nitrogen-cycle";
      const forumLink = process.env.CLIENT_URL ? `${process.env.CLIENT_URL}/community` : "http://localhost:8080/community";
	await sendEmail(user.email, "Welcome to The Aqua Guide!", "welcome.html", {
        name: user.name,
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    // Create reset link
    // Assuming frontend is running on process.env.CLIENT_URL or defaulting to localhost:8080
  ...
  try {
    ...
    const user = await User.findOne({
      where: {
        ...
        resetPasswordExpires: { [Op.gt]: Date.now() }, // Check if expiry is greater than now
      },
    });
